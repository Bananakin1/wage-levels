<script>
  ...
  for (const g of Object.values(geography)) {
    for (const c of g.counties) {
      statesFromCounties.add(c.fips.substring(0, STATE_FIPS_LENGTH));
    }
  }
  ...
  const stateItems = [
    ...
    ...[...statesFromCounties]
      ...
      .map(fips => {
        ...
        return { value: fips, label: geo?.state || STATE_NAMES[ab] || ab };
      })
  ...
  let stateFips = $derived(AB_TO_FIPS[filters.stateAb] || '');

  function onSearch() {
    const q = searchText.toLowerCase().trim();
    if (q.length < 2) { searchResults = []; showResults = false; return; }
    searchResults = occupationList
      .filter(o => {
  ...
  function handleSearchBlur() {
    ...
    setTimeout(() => { showResults = false; }, BLUR_HIDE_DELAY_MS);
  }
</script>
